...
public class StarPainter extends DevicePainter {
	...
	public StarPainter(DiagramElement device) {
		...
		((GeneralPath) shape).lineTo(0
				+ star.getSize().width, 0 + 18);

		((GeneralPath) shape).lineTo(0
				+ star.getSize().width / 4, 0 + 18
				+ star.getSize().height / 2);

		((GeneralPath) shape).lineTo(0
				+ star.getSize().width / 2, 0 + 18
				- star.getSize().height / 2.5);

		((GeneralPath) shape).lineTo(0
				+ star.getSize().width / 1.3, 0 + 18
				+ star.getSize().height / 2);
